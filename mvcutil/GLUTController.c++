...
int GLUTController::mapMods(int glutMods)
{
	int controllerMods = 0;
	if ((glutMods & GLUT_ACTIVE_SHIFT) != 0)
		controllerMods = MVC_SHIFT_BIT;
	if ((glutMods & GLUT_ACTIVE_CTRL) != 0)
		controllerMods |= MVC_CTRL_BIT;
	if ((glutMods & GLUT_ACTIVE_ALT) != 0)
...
void GLUTController::mouseFuncCB(int button, int state, int x, int y)
{
	...
	else
	{
		...
		if (Controller::curController != NULL)
			c->handleMouseButton(
				mButton, (state == GLUT_DOWN), x, y, mapMods(glutGetModifiers()));
	}
}
...
void GLUTController::specialKeyCB(int key, int x, int y)
{
	if (Controller::curController != NULL)
	{
		...
		if (haveSpecialKey)
			theC->handleSpecialKey(spKey, x, y, mapMods(glutGetModifiers()));
		else if ((key >= GLUT_KEY_F1) && (key <= GLUT_KEY_F12))
		{
			fcnKey = key - GLUT_KEY_F1 + 1;
			theC->handleFunctionKey(fcnKey, x, y, mapMods(glutGetModifiers()));
		}
	}
}
