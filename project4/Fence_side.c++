...
GLuint Fence_side::indexList_xmin[4] = { 6, 7, 0, 1 };
GLuint Fence_side::indexList_ymin[4] = { 6, 0, 4, 2 };
GLuint Fence_side::indexList_ymax[4] = { 1, 7, 3, 5 };

Fence_side::Fence_side(float cx, float cy, float cz, 
...
void Fence_side::defineFence_side(vec2* tex_Coords)
{
...
GLuint Fence_side::defineTexture(const char* texImageSource)
{
...
void Fence_side::getMCBoundingBox(double* xyzLimits) const
{
	//Not setting the limits, because we want this to continue out of view
	xyzLimits[0] = xmin;
	xyzLimits[1] = xmax;
	xyzLimits[2] = ymin;
	xyzLimits[3] = ymax;
...
// Establish current texture and any modified properties
	...
	// tell fragment shader we are rendering the Fence_side
	glUniform1i(ppuLoc_hasTex, 1);

	// faces we want to texture
